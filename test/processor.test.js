...
test('Robot can move forward to a new position', () => {
    const input = '5 3\n 1 1 E\n RFRFRFRF\n 3 2 N\n FRRFLLFFRRFLL\n 0 3 W\n LLFFFLFLFL'

    const state = gridMapper.mapGrid(input)
    const process = processor.process(state)
    const startingPosition = process.robots[0].position.y
    const newPosition = process.robots[1].position.y

    expect(newPosition ).not.toEqual( startingPosition);
});

test('Robot can move right to a new position', () => {
    const input = '5 3\n 1 1 E\n RFRFRFRF\n 3 2 N\n FRRFLLFFRRFLL\n 0 3 W\n LLFFFLFLFL'

    const state = gridMapper.mapGrid(input)
    const process = processor.process(state)
    const startingPosition = process.robots[0].position.x
    const newPosition = process.robots[1].position.x
